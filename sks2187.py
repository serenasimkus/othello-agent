...
class StudentEngine(Engine):
    ...
    def get_move(self, board, color, move_num=None,
        ...
        if (self.alpha_beta == False): 
            ...
            if len(moves) > 18:
                m = max(moves, key=lambda move: self.max_val_a_b(board, color, color, move, 3, time_remaining))
                if self.time:
                    self.end_time = timeit.default_timer()
                    self.total_avg += (self.end_time - self.start_time)
                    print self.avg_time()
                ...
                m = max(moves, key=lambda move: self.max_val_a_b(board, color, color, move, 4, time_remaining))
                if self.time:
                    self.end_time = timeit.default_timer()
                    self.total_avg += (self.end_time - self.start_time)
                    print self.avg_time()
    ...
    def max_val(self, board, color, original_color, move, depth, time_remaining):
        ...
        new_moves = newboard.get_legal_moves(color)
        self.num_elem += 1
    ...
    def max_val_a_b(self, board, color, original_color, move, depth, time_remaining):
        time_spent = timeit.default_timer() - self.start_time
        ...
        for move in new_moves:
            v = max(v, self.min_val_a_b(newboard, color*-1, original_color, move, depth-1, time_remaining))
            if v >= self.b:
                return v
            self.a = max(self.a, v)
    ...
    def min_val_a_b(self, board, color, original_color, move, depth, time_remaining):
        time_spent = timeit.default_timer() - self.start_time
        ...
        for move in new_moves:
            v = min(v, self.max_val_a_b(newboard, color*-1, original_color, move, depth-1, time_remaining))
            if v <= self.a:
                return v
            self.b = min(self.b, v)
